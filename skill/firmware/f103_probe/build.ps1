...
$elf = Join-Path $out 'f103_probe.elf'
$bin = Join-Path $out 'f103_probe.bin'
$hex = Join-Path $out 'f103_probe.hex'
...
$gccArgs += @('-Wl,-Map,' + (Join-Path $out 'f103_probe.map'))
$gccArgs += $sources
# -nostdlib also suppresses libgcc, and this firmware does real float math,
# so the soft-float helpers (__aeabi_f*) must be linked explicitly.
$gccArgs += @('-lgcc')
$gccArgs += @('-o', $elf)
